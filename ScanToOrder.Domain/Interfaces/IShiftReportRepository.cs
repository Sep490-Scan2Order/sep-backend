...
namespace ScanToOrder.Domain.Interfaces
{
    ...
    {
        Task<(List<(ShiftReport Report, string CashierName)> Items, int TotalCount)> GetReportsByRestaurantAsync(
            int restaurantId, DateTime? from, DateTime? to, int pageIndex = 1, int pageSize = 10);

        Task<(List<(ShiftReport Report, string CashierName)> Items, int TotalCount)> GetReportsByStaffAsync(Guid staffId, int pageIndex = 1, int pageSize = 10);
